...
# Setup CDM using direct connection for CohortGenerator compatibility
cdm_setup <- setup_omop_cdm_eunomia_direct()

# Create cohort tables first
createCohortTables(
  connectionDetails = cdm_setup$connectionDetails,
  cohortDatabaseSchema = "main",
  cohortTableNames = CohortGenerator::getCohortTableNames()
)

# First, check what conditions are available in the dataset
available_conditions <- DatabaseConnector::querySql(
  cdm_setup$connection,
  "SELECT condition_concept_id, COUNT(*) as patient_count
...
# Use the most common condition for the cohort
most_common_condition <- available_conditions$CONDITION_CONCEPT_ID[1]

# Define a cohort using the most common condition
cohortDefinitionSet <- data.frame(
  cohortId = 1,
  cohortName = paste("Condition", most_common_condition),
  sql = paste0("SELECT DISTINCT person_id, condition_start_date as cohort_start_date, 
                condition_end_date as cohort_end_date
                FROM condition_occurrence 
                WHERE condition_concept_id = ", most_common_condition)
)
...
# Query the generated cohort
cohort_counts <- DatabaseConnector::querySql(
  cdm_setup$connection,
  "SELECT cohort_definition_id, COUNT(*) as subject_count 
   FROM cohort 
   GROUP BY cohort_definition_id"
)
print(cohort_counts)

# Get cohort subjects for further analysis
diabetes_patients <- DatabaseConnector::querySql(
  cdm_setup$connection,
  "SELECT subject_id, cohort_start_date, cohort_end_date 
   FROM cohort 
   WHERE cohort_definition_id = 1"
)
head(diabetes_patients)

# Analyze demographics of cohort subjects
demographics <- DatabaseConnector::querySql(
  cdm_setup$connection,
...
# Clean up
DatabaseConnector::disconnect(cdm_setup$connection)
```
